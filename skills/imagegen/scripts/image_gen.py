...
def _image_item_to_bytes(item: Any) -> bytes:
    """Return raw image bytes from one API image item.

    Prefers inline base64 (``b64_json``). Falls back to downloading ``url`` for
    proxies/gateways that hand back a hosted image URL with an empty
    ``b64_json`` (without this, callers would silently write a 0-byte file).
    Raises if neither source yields data so failures are never silent.
    """
        ...
        for attempt in range(3):
            try:
                req = urllib.request.Request(
                    url, headers={"User-Agent": "Mozilla/5.0 (imagegen-cli)"}
                )
                with urllib.request.urlopen(req, timeout=180) as resp:
                    data = resp.read()
                if data:
                    return data
                last_exc = RuntimeError("downloaded 0 bytes")
            except Exception as exc:  # noqa: BLE001 - retry transient download errors
                last_exc = exc
            if attempt < 2:
                time.sleep(2.0 * (attempt + 1))
        raise RuntimeError(f"Failed to download image URL after retries: {url} ({last_exc})")
    raise RuntimeError("Image response contained neither b64_json nor url")


def _decode_and_write(images: List[bytes], outputs: List[Path], force: bool) -> None:
    for idx, raw in enumerate(images):
        ...
        if not raw:
            _die(f"Refusing to write empty image to {out_path} (API returned no image data)")
...
def _is_transient_error(exc: Exception) -> bool:
    ...
    if "timeout" in msg or "timed out" in msg or "connection reset" in msg:
        return True
    # 网关/上游瞬时错误（502/503/504），常见于中转代理，应自动重试
    if any(s in msg for s in ("502", "503", "504", "bad gateway", "service unavailable", "gateway timeout", "upstream")):
        return True
...
async def _run_generate_batch(args: argparse.Namespace) -> int:
    ...
    async def run_job(i: int, job: Dict[str, Any]) -> Tuple[int, Optional[str]]:
        ...
        try:
            ...
            images = [_image_item_to_bytes(item) for item in result.data]
